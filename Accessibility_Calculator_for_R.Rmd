...
departure_datetime <- as.POSIXct("2021-05-04 08:00:00",  # must be within calendar range of gtfs 
                                 format = "%Y-%m-%d %H:%M:%S", 
...
accessibility <- ttm %>%
  ...
  # mutate
  mutate(POW2_0 = o_j * case_when(travel_time < 1 ~ 1, TRUE ~ travel_time^-2),
         EXP0_15 = o_j *  2.71828^(-0.15*travel_time),
         MGAUS180 = o_j * 2.71828^(-travel_time^2/180),
         CUMR40 = o_j * case_when(travel_time <= 40 ~ 1, TRUE ~ 0),
         CUML40 = o_j * case_when(travel_time <= 40 ~ 1-travel_time/40, TRUE ~ 0)) %>%
  
  # summarize results
  group_by(fromId) %>%
  summarize(POW2_0 = sum(POW2_0),
            EXP0_15 = sum(EXP0_15),
            MGAUS180 = sum(MGAUS180),
            CUMR40 = sum(CUMR40),
            CUML40 = sum(CUML40)) %>%
